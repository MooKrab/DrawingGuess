import pygame
from libs.common.components import SolidButton, SolidSlider, ImageButton
...
def surface(screen: pygame.Surface, background: pygame.Surface, open_file_on_start: bool = False, /) -> None:
    ...
    # Top bar buttons
    # --- MODIFIED: Replaced "Menu" text with icon_path AND corrected path ---
    menu_btn: SolidButton = SolidButton(
        0, 0, FILE_BTN_WIDTH, TOP_BAR_HEIGHT, 
        text="Meun",
    ...
    history_btn: SolidButton = SolidButton(
        menu_btn.rect.right, 0, HISTORY_BTN_WIDTH, TOP_BAR_HEIGHT, "History", 
        font_size=20, bg_color=MENU_BG_COLOR, text_color=MENU_TEXT_COLOR, 
        border_width=0, border_color=None
    )
    ...
    file_menu_hot_zone: List[pygame.Rect] = [menu_btn.rect]
    ...
    def set_dialog(state: Optional[str], pending_action: Optional[str] = None) -> None:
        ...
        if state == "confirm_action":
            btn_w, btn_h, btn_gap = 140, 40, 20 # Increased gap slightly
            
            # --- MODIFIED: Only two buttons ---
            quit_btn = SolidButton(0, 0, btn_w, btn_h, "Quit", font_size=20, bg_color=(200, 0, 0), text_color=(255, 255, 255))
            cancel_btn = SolidButton(0, 0, btn_w, btn_h, "Cancel", font_size=20, bg_color=(150, 150, 150), text_color=(0, 0, 0))
            
            # Center-align the two buttons
            total_w: int = btn_w * 2 + btn_gap
            start_x: int = dialog_rect.centerx - total_w // 2
            btn_y: int = dialog_rect.bottom - btn_h - 20
            
            quit_btn.rect.topleft = (start_x, btn_y)
            cancel_btn.rect.topleft = (start_x + btn_w + btn_gap, btn_y)
            
            dialog_buttons = [
                {"name": "quit", "btn": quit_btn},
    ...
    while running:
        ...
        if dialog_state is not None:
            shared_tool_context["click_on_ui"] = True # We are interacting with UI
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if action_taken:
                        if action_taken == "cancel":
                            ...
                            # User clicked "Quit" (discards changes)
                            # Perform the pending action without saving
                            dialog_state = None
                            action = dialog_pending_action
                            dialog_pending_action = None
                            
                            if action == "exit":
                                running = False
                                continue # Exit main loop
                            elif action == "new_canvas":
                                clear_canvas_action()
                            elif action == "open_file":
                                open_file_action()

                        elif action_taken == "export":
                            # This is from a different dialog, but we leave it
                            export_as_image_action()
                    
                    if event in events: events.remove(event)

                # Also handle clicking the 'X' on the window
            ...
            for event in events[:]: # Iterate over a copy
                ...
                if event.type == pygame.MOUSEMOTION:
                    if shared_tool_context["menu_open"] is not None:
                        ...
                        if menu_btn.rect.collidepoint(mouse_pos) and shared_tool_context["menu_open"] != "file":
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    # --- File Menu Clicks ---
                        ...
                        file_menu_hot_zone = [menu_btn.rect]
                        btn_y: int = menu_btn.rect.bottom
                        btn_w: int = 300
                        btn_h: int = 40
                        ...
                        def add_file_btn(text: str) -> None:
                            ...
                            btn: SolidButton = SolidButton(
                                menu_btn.rect.left, btn_y, btn_w, btn_h, text, 
                        ...
                        # --- MODIFIED: Removed other menu options ---
        ...
        # --- MODIFIED: Draw highlights *BEFORE* the buttons ---
        if shared_tool_context["menu_open"] == "file":
            highlight_rect: pygame.Rect = menu_btn.rect.inflate(-8, -8)
        ...
        # --- MODIFIED: Call draw() for buttons (now drawn on top of highlight) ---
        menu_btn.draw(screen)
        history_btn.draw(screen)
        
        # --- DELETED: Old manual blit calls ---
        # screen.blit(menu_btn.text_surf, menu_btn.text_rect)
